 ...
 * * Uso:
 * ./image_horizontal <caminho_para_imagem.pgm> <n_threads>
 */
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
int main(int argc, char **argv) {
    ...
    printf("=== VERSAO: CONCORRENTE - FATIAS HORIZONTAIS ===\n");
    printf("Imagem %s (%dx%d) | threads=%d\n", nome_arquivo, img->width, img->height, threads);
    printf("[HORIZONTAL] Tempo de I/O (leitura): %.3f ms\n", ms_io);
    ...
    for (int i = 0; i < threads; i++) {
        int ini = y;
        int linhas_extra = (i < resto ? 1 : 0);
        int fim = ini + linhas_base + linhas_extra;
    ...
    printf("[HORIZONTAL] Tempo de Computação: %.3f ms (%.6f s)\n", ms_comp, ms_comp/1000.0);
