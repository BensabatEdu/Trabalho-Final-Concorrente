/*
 * src/image_tiling_static.c
 * Compilar:
 * gcc src/image_tiling_static.c -o image_tiling_static -lpthread -O3 -Wall -Wextra
 * * Uso:
 * ./image_tiling_static <arquivo.pgm> <n_threads> [largura_bloco] [altura_bloco]
 */
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < n_threads; i++) {
        int ini = cur;
        int blocos_extra = (i < resto ? 1 : 0);
        int fim = ini + base + blocos_extra;
        ...
        pthread_create(&ths[i], NULL, thread_func_static, &args[i]);
        cur = fim;
    }
    ...
    printf("[TILING ESTATICO] Contador final = %lld\n", g_contador_global);
    printf("[TILING ESTATICO] Tempo de Computação: %.3f ms (%.6f s)\n", ms_comp, ms_comp/1000.0);
